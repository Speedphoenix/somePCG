...
struct Tile
{
	...
	void med(const std::vector<Tile>& tab, bool useatten = true)
	{
		E(usedAttenuation)
		E(tab.size())
		E(tab.size() + (useatten ? (1 / usedAttenuation) : 1))

		for (const auto& elem : tab)
			for (int i = 0;i < VALNB;i++)
				vals.at(i) += elem.vals.at(i);
		for (int i = 0;i < VALNB;i++)
			vals.at(i) /= (tab.size() + (useatten ? 0 /*(1 / usedAttenuation)*/ : 1));
	}
	...
	ALLEGRO_COLOR getcol() const
	{
		...
		break;
			case 2:
			return (al_map_rgb(vals.at(0), vals.at(1), (vals.at(0) + vals.at(1)) / 2));
		break;
			default:
			case 3:
...
ALLEGRO_BITMAP *showResult(std::map<int, std::map<int, Tile>>& sol);
void pauseUntilEsc();
